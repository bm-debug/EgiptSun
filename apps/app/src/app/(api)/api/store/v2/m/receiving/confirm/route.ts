...
/**
 * POST /api/store/v2/s/receiving/confirm
 * Confirm receiving base move (only manager)
 * Body: { full_baid }
 */
async function handlePost(context: AuthenticatedRequestContext) {
  ...
  try {
    ...
    try {
      const result = await repository.confirmReceiving({
    ...
    console.error('Confirm receiving error:', error)
    return new Response(
      JSON.stringify({ 
        error: 'Не удалось подтвердить входящую машину',
