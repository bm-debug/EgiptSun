...
import { Loader2, CalendarIcon, Plus, Trash2, Send, ArrowLeft, CheckCircle } from 'lucide-react'
import { cn } from '@/lib/utils'
import { format } from 'date-fns'
import { ru } from 'date-fns/locale'
import { ManagerAddEditSendingItem } from '@/packages/components/blocks-app/m/ManagerAddEditSendingItem'
...
export default function SendingEditPage() {
  ...
  useEffect(() => {
    const fetchContractors = async () => {
      ...
      try {
        const response = await fetch('/api/store/v2/m/contractors?page=1&limit=1000', {
          credentials: 'include',
        })
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `/api/store/v2/m/sending/details?full_baid=${encodeURIComponent(fullBaid)}`,
        { credentials: 'include' }
      )
  ...
  const handleConfirmSending = async () => {
    ...
    setSaving(true)
    try {
      const response = await fetch('/api/store/v2/m/sending/confirm', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const canConfirm = data.statusName === 'ON_APPROVAL'
    ...
    <div className="Container mx-auto py-4 md:py-6 space-y-4 md:space-y-6 px-4 md:px-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 md:gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push('/m/sending')}>
