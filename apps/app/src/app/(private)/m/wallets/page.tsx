...
export default function Page() {
  ...
  const fetchData = useCallback(async (withLoader = true) => {
    ...
    try {
      const response = await fetch(`/api/store/v2/m/wallets?page=${page}&limit=${PAGE_SIZE}`, {
        credentials: 'include',
      })
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/store/v2/m/wallet-transactions', {
        method: 'POST',
        headers: {
