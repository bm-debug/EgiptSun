...
export interface AIRepositoryConfig {
  env: {
    AI_API_URL: string;
    AI_API_TOKEN: string;
    BOT_TOKEN?: string; // Optional, only needed for voice transcription
...
export interface RecentMessage {
  title: string;
  data_in: string | object | null; // Can be JSON string or parsed object
}

export interface MessageToSummarize {
  title: string;
  data_in: string | object | null;
  full_maid: string;
  created_at: string;
}
        ...
        for (const msg of messagesToProcess) {
          ...
          try {
            ...
            if (msg.data_in) {
              ...
              if (typeof msg.data_in === 'string') {
                dataInObj = JSON.parse(msg.data_in);
              } else {
                dataInObj = msg.data_in;
              }
              ...
              if (dataInObj.data) {
                try {
                  const dataObj = typeof dataInObj.data === 'string' 
                    ? JSON.parse(dataInObj.data) 
                    : dataInObj.data;
