...
class I18nService {
  ...
  /**
   * Fetch translations from API or direct import
   */
  private async fetchTranslations(locale: string): Promise<Translations> {
    // Try API first (client-side)
    if (typeof window !== 'undefined') {
      try {
  ...
  /**
   * Get translation value by key path (e.g., "dataTable.loading" or "navigation.home")
   */
   ...
   * @param key Translation key (supports dot notation, e.g., "dataTable.loading")
