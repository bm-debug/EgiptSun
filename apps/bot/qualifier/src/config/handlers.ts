...
export const createCustomHandlers = (worker: BotInterface) => {
  const handlerWorker = {
    ...
    textRepository: worker['textRepository'],
  ...
  return {
    ...
    handleStartCommand: async (message: any, bot: any) => {
      ...
      await handlerWorker.flowEngine.startFlow(userId, 'start');
...
${aiResponse}`
    ...
    /**
     * Load next qualification step from database
     * This handler loads qualification steps dynamically and processes user input
     */
    loadNextOnboardingStep: async (telegramId: number, contextManager: UserContextManager) => {
      console.log(`🔄 Loading next qualification step for user ${telegramId}`);
      
      try {
        // Get current step index from context (starts from 0)
        // If not set, initialize to 0 (first step will be order = 1)
        let currentStepIndex = await contextManager.getVariable(telegramId, 'qualification.currentStepIndex');
        if (currentStepIndex === null || currentStepIndex === undefined) {
          // First time - reset to 0 and initialize answers array
          currentStepIndex = 0;
          await contextManager.setVariable(telegramId, 'qualification.currentStepIndex', 0);
          await contextManager.setVariable(telegramId, 'qualification.answers', []);
          console.log(`🆕 Initializing qualification step index to 0 and answers array`);
        }
        const stepIndex = Number(currentStepIndex);
        
        console.log(`📊 Current step index: ${stepIndex}`);
        
        // Load next step from database
        const nextStep = await handlerWorker.textRepository.getQualificationStepByOrder(stepIndex + 1);
        
        if (!nextStep) {
          // No more steps - complete qualification
          console.log(`✅ No more qualification steps, completing qualification for user ${telegramId}`);
          
          // Reset qualification state for next time
          await contextManager.setVariable(telegramId, 'qualification.completed', false);
          await contextManager.setVariable(telegramId, 'qualification.currentStepIndex', 0);
        ...
        // Parse data_in to get save_to_variable, validation, and buttons
        let saveToVariable = 'qualification.answer_' + (stepIndex + 1);
        let validation: any = undefined;
        let buttons: any[] | undefined = undefined;
        
        if (nextStep.dataIn) {
          try {
            const dataIn = JSON.parse(nextStep.dataIn);
            if (dataIn.save_to_variable) {
              // Replace old 'onboarding.' prefix with 'qualification.' if present
              saveToVariable = dataIn.save_to_variable.replace(/^onboarding\./, 'qualification.');
            }
            if (dataIn.validation) {
              validation = dataIn.validation;
            }
            if (dataIn.buttons && Array.isArray(dataIn.buttons) && dataIn.buttons.length > 0) {
              buttons = dataIn.buttons;
            }
          } catch (e) {
            console.warn(`Failed to parse data_in for step ${stepIndex + 1}, using defaults`);
          }
        }
        
        // Get question text from content field
        const questionText = nextStep.content || nextStep.title || 'Введите ответ:';
        
        console.log(`📝 Step ${stepIndex + 1}: "${questionText.substring(0, 50)}..."`);
        console.log(`💾 Saving to variable: ${saveToVariable}`);
        if (buttons) {
          console.log(`🔘 Step has ${buttons.length} buttons`);
        }
        
        // Update step index in context
        await contextManager.setVariable(telegramId, 'qualification.currentStepIndex', stepIndex + 1);
        await contextManager.setVariable(telegramId, 'qualification.currentStep', {
        ...
        // Get context for sending message
        const context = await contextManager.getContext(telegramId);
        if (!context) {
          console.error(`❌ Context not found for user ${telegramId}`);
          return;
        }
        
        // If step has buttons, create keyboard and send with buttons
        if (buttons) {
          // Create keyboard with special callback_data prefix for qualification
          const callbackPrefix = `qualification_step_${stepIndex + 1}`;
          const keyboard = {
            inline_keyboard: [
              buttons.map(button => ({
                text: button.text,
                callback_data: `${callbackPrefix}_${button.callback_data}`
              }))
            ]
          };
          
          // saveToVariable is already stored in qualification.currentStep
          // No need for temporary variable - callback handler will use currentStep.saveToVariable
          
          // Send question with buttons
          if (context.flowInTopic && context.topicId && context.adminChatId) {
            await handlerWorker.messageService.sendMessageWithKeyboardToTopic(
              context.adminChatId,
              context.topicId,
              questionText,
              keyboard
          ...
          console.log(`✅ Question with buttons sent, waiting for button click...`);
        } else {
          // No buttons - set up wait input state for text input
          await contextManager.setVariable(telegramId, '_system.waitingForInput', {
            stepId: 'load_next_onboarding_step',
            saveToVariable: saveToVariable,
            validation: validation,
            nextStepId: 'load_next_onboarding_step' // Loop back to this handler
          });
          ...
          console.log(`✅ Question sent, waiting for user input...`);
        }
        
      } catch (error) {
        console.error(`❌ Error in loadNextOnboardingStep:`, error);
        
        // Try to send error message
        try {
          const context = await contextManager.getContext(telegramId);
          if (context) {
            const errorMessage = 'Произошла ошибка при загрузке вопроса. Попробуйте позже.';
            if (context.flowInTopic && context.topicId && context.adminChatId) {
              await handlerWorker.messageService.sendMessageToTopic(
                context.adminChatId,
                context.topicId,
