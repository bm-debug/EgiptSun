...
export default function PartnerAuthGuard({ children }: PartnerAuthGuardProps) {
  ...
  const checkUserAuth = async () => {
    try {
      ...
      const hasPartnerRole = data.user.roles.some(
        (role) => role.name === 'Partner' || role.name === 'Партнер'
      )
      const isAdmin = data.user.roles.some((role) => role.name === 'Administrator')

      return hasPartnerRole || isAdmin
    } catch (err) {
