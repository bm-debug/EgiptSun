...
export function ManagerAddEditSendingItem({ 
  open, 
  onOpenChange, 
  fullBaid,
  item,
  ...
  const [sellingPriceFact, setSellingPriceFact] = useState<string>('')
  const [purchasePriceFact, setPurchasePriceFact] = useState<string>('')
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/store/v2/m/products?${params}`, {
          credentials: 'include',
        })
  ...
  useEffect(() => {
    if (item && open) {
      ...
      setSellingPriceFact(item.sellingPriceFact ? String(item.sellingPriceFact) : '')
      setPurchasePriceFact(item.purchasePriceFact ? String(item.purchasePriceFact) : '')
    }
  }, [item, open])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        sellingPriceFact: sellingPriceFact ? parseFloat(sellingPriceFact) : undefined,
        purchasePriceFact: purchasePriceFact ? parseFloat(purchasePriceFact) : undefined,
        notes: notes || undefined,
        ...(isEdit && item ? { itemUuid: item.uuid } : {}),
      }

      const response = await fetch('/api/store/v2/m/sending/details', {
        method: 'POST',
        headers: {
      ...
      <DialogContent className="sm:max-w-[500px] max-h-screen h-screen sm:h-auto p-0 gap-0 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="space-y-4 py-4 px-4 sm:px-6 flex-1 overflow-y-auto">
            ...
            <div className="space-y-2">
              <Label htmlFor="sellingPriceFact">
                Цена отпуска (факт)
              </Label>
              <Input
                id="sellingPriceFact"
                ...
                min="0"
                step="0.01"
                value={sellingPriceFact}
                onChange={(e) => setSellingPriceFact(e.target.value)}
                disabled={loading}
            ...
            <div className="space-y-2">
              <Label htmlFor="purchasePriceFact">
                Цена закупки (факт)
              </Label>
              <Input
                id="purchasePriceFact"
                type="number"
                min="0"
                step="0.01"
                value={purchasePriceFact}
                onChange={(e) => setPurchasePriceFact(e.target.value)}
                disabled={loading}
                placeholder="Опционально"
              />
            </div>
