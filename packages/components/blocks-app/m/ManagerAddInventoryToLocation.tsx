...
interface AddInventoryToLocationProps {
  ...
  onSuccess?: () => void
  selectedLocations?: string[] // Optional location filters from parent page
}

export function ManagerAddInventoryToLocation({ 
  open, 
  onOpenChange, 
  onSuccess,
  selectedLocations = []
  ...
  const locationLaid = managerData?.location?.laid
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        // Use location filters from parent page if available
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      ...
      try {
        const response = await fetch('/api/store/v2/m/inventory-statuses', {
          credentials: 'include',
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/store/v2/m/inventory/add', {
        method: 'POST',
        headers: {
          ...
          <div className="space-y-4 py-4 px-4 sm:px-6 flex-1 overflow-y-auto">
            ...
            {locationLaid && (
              <div className="rounded-lg border border-muted bg-muted/50 p-3 text-sm text-muted-foreground">
                📦 Склад: {managerData?.location?.title || locationLaid}
              </div>
            )}
