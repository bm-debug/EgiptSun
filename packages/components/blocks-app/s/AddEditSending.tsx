...
export function AddEditSending({ open, onOpenChange, uuid, onSuccess }: AddEditSendingProps) {
  ...
  useEffect(() => {
    const fetchExistingData = async () => {
      ...
      setLoading(true)
      try {
        const response = await fetch(`/api/store/v2/s/sending/${uuid}`, {
          credentials: 'include',
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit 
        ? `/api/store/v2/s/sending/${uuid}`
        : '/api/store/v2/s/sending'
