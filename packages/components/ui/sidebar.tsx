...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => {
    // If openProp is provided, use it; otherwise use initial state from cookie
    return openProp !== undefined ? openProp : getInitialOpenState()
  })
  
  // Update internal state when openProp changes (controlled mode)
  React.useEffect(() => {
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          style={
            {
              ...(resizable ? {} : { "--sidebar-width": SIDEBAR_WIDTH }),
...
function SidebarRail({ className, onMouseDown, ...props }: React.ComponentProps<"button"> & { onMouseDown?: (e: React.MouseEvent) => void }) {
  const { toggleSidebar, state } = useSidebar()
  
  // Only show resize functionality when sidebar is expanded
  const showResize = state === "expanded"
  
  const handleMouseDown = (e: React.MouseEvent) => {
    if (showResize && onMouseDown) {
      onMouseDown(e)
    }
  }
  ...
  return (
    <button
      ...
      aria-label={showResize ? "Resize Sidebar" : "Toggle Sidebar"}
      tabIndex={-1}
      onClick={showResize ? undefined : toggleSidebar}
      onMouseDown={handleMouseDown}
      title={showResize ? "Resize Sidebar" : "Toggle Sidebar"}
      className={cn(
        "hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
        showResize ? "cursor-col-resize" : "cursor-pointer",
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "md:ms-[--sidebar-width] md:peer-data-[state=collapsed]:ms-[--sidebar-width-icon]",
